...
# Hitung CV per komoditas (dari data bulanan)
cv_list = []
for kom in list(komoditas_singkat.keys()):
...
ax.axvline(df_cv["CV_%"].mean(), color="gray", linestyle="--", alpha=0.7, label=f"Rata-rata CV = {df_cv['CV_%'].mean():.1f}%")
